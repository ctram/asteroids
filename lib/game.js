(function () {
  ...
  var Game = Asteroids.Game = function (options) {
    ...
    this.canvasEl.addEventListener('mousemove', function(evt) {
      ...
      var diffAngle = this.angleDiffMouse(this.ship.pos, this.mousePos);


      var mousePos = this.getMousePos(this.canvasEl, evt);
      this.mousePos = mousePos;
      this.message = 'Mouse position: X: ' + mousePos.x + ' Y: ' + mousePos.y + ' Angle: ' + diffAngle;
  ...
  Game.prototype.angleDiffMouse = function (orgPos, mousePos) {
    var mouseX = mousePos[0];
    var mouseY = mousePos[1];
    var orgX = orgPos[0];
    var orgY = orgPos[1];

    var deltaX = mouseX - orgX;
    var deltaY = mouseY - orgY;

    var diffAngle = Math.atan(deltaY / deltaX) * 180 / Math.PI + 90;
